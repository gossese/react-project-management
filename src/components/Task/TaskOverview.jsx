  ...
  return (
    <article>
      ...
      <div className="flex gap-4">
        <Input error={taskError} className="w-64" onChange={(event)=> setEnteredTask(event.target.value)} value={enteredTask} ref={inputRef}></Input>
        ...
        {taskList.filter((task)=> task.prId === project.id).length > 0 ? (
          ...
          {taskList.map((task, index) => {
            if (task.prId === project.id) {
              return (
                <Task key={task.tId} deleteTask={() => handleDeleteTask(index)}>
                  {task.tName}
                </Task>
              );
            }
